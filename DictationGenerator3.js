...
function chordPrevention (prevChord, newChord) {
	
	if (newChord === prevChord) {
		return true; //Can't be the same twice in a row//
	}
	...
	if (newChord === "vii06" && (prevChord === "viiø7" ||prevChord === "viiø6/5" || prevChord === "viiø4/3" || prevChord === "viiø4/2")) {
		return true; //No triad after 7th
	}
	...
	// Make into an array! Code works now, which is great, but that would make it more organized.
}
...
//This defines the function
	...
	//Standard for loop, generates 6 numbers 
		...
		do {
			chordNumber = Math.floor(Math.random() * 3); // 0 1 or 2: tonic, predominant, and dominant
			
			var prevRoman = null;
			if (romanNumeralList.length > 0 ) {
 				prevRoman = romanNumeralList[romanNumeralList.length - 1 ];
			}// Get previous Roman Numeral
						
			//if (i > 0 && chordPrevention(romanNumeralList[i - 1], chordNumber)) {
			if (i > 0 && romanNumeralList[i - 1]) {
				continue;
			}
			
			if (i > 0 && chordNumber === 1 && chordList[i - 1] === 2) {
				continue; //Prevents a predominant follows a dominant
			}
			...
			if (i > 1 && chordNumber === chordList[i - 1] && chordNumber === chordList[i - 2]) {
				continue; // If the three numbers are equal, a different number is generated.
			}
			if ( i === 5 && chordList[4] === 2 && chordNumber === 2) {
				continue; // Ensures that 5 and 6 are both not 2. 
			}
	...
	if (randomNum < 0.7) {
		finalCadence = [2, 0]; //70% PAC/IAC
	} else if (randomNum < 0.9) {
		finalCadence = [1,0]; //20% Plagal
	} else {
		finalCadence = [1, 2]; //10% HC
	}	
	...
	for (var i = 0; i < chordList.length; i++) {
		...
		if (i === 0) {
			...
			if (num === 0 ) {
				...
				do {
 					chosenRoman = options[Math.floor(Math.random() * options.length)];
				} while (chordPrevention(prevRoman, chosenRoman));
					
				//chordPrevention on chosenRoman in each block of code, if chordPrevention returns true, it needs to go back (loop) until chordPrevention is false = good
				//everytime chosenRoman is assigned
			}
		} else if (i === 5) {
			if (num === 0) {
				var filteredOptions = [];
				for (var j = 0; j < options.length; j++) {
					if (options[j] !== "Cad6/4") {
						filteredOptions.push(options[j]); //Adds Cad6/4
					}
				}
			...
			do {
				chosenRoman = options[Math.floor(Math.random() * options.length)];
			} while (i > 0 && chordPrevention(romanNumeralList[i - 1], chosenRoman)); //Ensures the same chord is not twice in a row.
		}
	}	
